...
exports.abide = function (options) {
  ...
  options.supported_languages.forEach(function (lang, i) {
    ...
    mo_cache[l] = {
      ...
      /* json_exists: exists(json_file_path(l)), */
      gt: null
    };
    if (l !== debug_locale) {
  ...
  return function(req, resp, next) {
    ...
    set_local = function (k, v) { resp.locals[k] = v;}; // Express 3.0
    if (resp.local && typeof resp.local === "function") { // Express 2.0 Support
        set_local = function (k, v) { resp.local (k, v)};
    }
    set_local('lang', lang);

    lang_dir = BIDI_RTL_LANGS.indexOf(lang) >= 0 ? 'rtl' : 'ltr';
    set_local('lang_dir', lang_dir);
    req.lang = lang;

    locale = localeFrom(lang);

    set_local('locale', locale);
    req.locale = locale;

    set_local('format', format);
    req.format = format;
    ...
    if (mo_cache[locale].mo_exists) {
      ...
      set_local(options.gettext_alias, gt.gettext.bind(gt));
      req.gettext = gt.gettext.bind(gt);
      set_local(options.ngettext_alias, gt.ngettext.bind(gt));
      req.ngettext = gt.ngettext.bind(gt);
   } else {
      // en-US in a non gettext environment... fake it
      var identity = function (a, b) { return a; };
      set_local(options.gettext_alias, identity);
      req.gettext = identity;
      set_local(options.ngettext_alias, identity);
