...
exports.abide = function (options) {
  ...
  console.log(options.supported_languages);
  ...
  console.log(req.headers['accept-language']);
    ...
    if (lang && lang.toLowerCase && lang.toLowerCase() == debug_lang) {
        console.log('debug localed matched');
        lang = 'db-LB'; // What? http://www.youtube.com/watch?v=rJLnGjhPT1Q
    } else {
        
        console.log(lang, ' is not ', debug_lang);
    }
    ...
    if (mo_cache[locale].mo_exists) {
      ...
      console.log('setting gettext into ', options.gettext_alias, gt);
      ...
      console.log('FAKING setting gettext into ', options.gettext_alias);
...
console.log(languages, supported_languages, defaultLanguage);
  ...
  for(var i=0; i < languages.length; i++) {
    var lq = languages[i];
    console.log(lower, ' contains ', lq.lang.toLowerCase());
